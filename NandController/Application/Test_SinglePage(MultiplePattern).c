/*
 *  Test_SinglePage(MultiplePattern).c
 *
 *  Created on: Mar 2, 2026
 *  Author: CPC
 */

#include "Test_SinglePage(MultiplePattern).h"
 ...
 * --------------------------------------------------------------------------- */
void MultiplePattern(uint32_t BlockNum)
{
	uint8_t readBuffer[PAGE_MAIN_SIZE];
	uint8_t writeBuffer[PAGE_MAIN_SIZE];
	uint32_t base_page = BlockNum * PAGES_PER_BLOCK;
