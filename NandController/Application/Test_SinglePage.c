...
void StandardTest(uint32_t BlockNum)
{
	uint8_t WriteBuffer[PAGE_MAIN_SIZE];
	uint8_t ReadBuffer[PAGE_MAIN_SIZE];
	uint32_t BasePage = BlockNum * PAGES_PER_BLOCK;
	 ...
	 */
	BlockErase128K_service(BlockNum, 500);
	 ...
	 */
	StandardProgram_Service(BasePage, WriteBuffer, PAGE_MAIN_SIZE);
	 ...
	 *      11 = Uncorrectable
	 */
	StandardRead_Service(BasePage, 0x0000, ReadBuffer, PAGE_MAIN_SIZE);
